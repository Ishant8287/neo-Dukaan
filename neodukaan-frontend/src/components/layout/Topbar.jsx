  ...
  const syncData = () => {
    ...
    if (savedShop) {
      try {
        ...
        const name = parsedShop.shopName || parsedShop.ownerName || "NeoDukaan";
        setShopName(name);
        setInitials(name.substring(0, 2).toUpperCase());
        setUserAvatar(parsedShop.avatar || null);
      } catch (error) {
        console.error("Failed to parse shop data", error.message);
      }
    }
  };
    ...
    <div className="h-16 border-b border-slate-800 bg-[#0b1120] flex items-center justify-between px-4 md:px-6 z-30 sticky top-0">
      <div className="flex items-center gap-4 min-w-0 flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-0 sm:gap-2 overflow-hidden">
          ...
          {/* 🚀 Dynamic Page Title yahan render ho raha hai */}
