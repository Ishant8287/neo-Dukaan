...
const POS = () => {
  ...
  const SHOP_NAME = shopProfile.shopName || shopProfile.name || "RetailFlow";
  ...
  const handleCheckoutClick = () => {
    ...
    if (
      Math.round((cash + upi + udhaar) * 100) !== Math.round(grandTotal * 100)
    )
      return toast.error(`Split must equal ₹${grandTotal.toFixed(2)}`);
    if (udhaar > 0 && customerPhone.trim().length < 10)
      return toast.error("10-digit phone required for Udhaar.");
