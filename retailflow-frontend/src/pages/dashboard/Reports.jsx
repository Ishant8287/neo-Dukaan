...
const Reports = () => {
    ...
    () =>
      filteredSales.reduce(
        (acc, sale) => {
          ...
          acc.udhaar += Number(sale.paymentSplit?.udhaar || 0);
          return acc;
        },
        { revenue: 0, profit: 0, gst: 0, udhaar: 0 },
      ),
    [filteredSales],
  );
  ...
  const paymentData = useMemo(
    () => [
      ...
      },
      {
        name: "Udhaar",
        value: filteredSales.reduce(
          (s, b) => s + Number(b.paymentSplit?.udhaar || 0),
          0,
        ),
      },
    ],
  ...
  return (
    <div className="text-white space-y-8 bg-transparent min-h-screen pb-20">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-[#111827] p-1.5 rounded-2xl border border-slate-800 shadow-inner">
          {["7d", "30d", "6m", "1y", "custom"].map((r) => (
            <button
              key={r}
              onClick={() => setRange(r)}
              className={`px-5 py-2 text-xs font-black rounded-xl transition-all ${range === r ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-slate-500 hover:text-white"}`}
            >
              {r.toUpperCase()}
            </button>
          ))}
